  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // FIXED: Saving both formats to ensure global layout Sidebar reads your real name
      ...
      <div className="max-w-md w-full bg-card border border-border p-8 rounded-2xl shadow-xl">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full bg-muted/40 border border-border rounded-lg px-4 py-3 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-slate-900/50 transition-all uppercase text-slate-900"
                required
              />
            </div>
        ...
        <div className="mt-8 text-center border-t border-border pt-6">
          <p className="text-sm text-muted-foreground font-medium">
            ...
            <button onClick={() => router.push("/signup")} className="text-slate-900 hover:underline font-bold">
              Register Account
            </button>
          </p>
        </div>
